...
public class GetLatestMessagesQueryHandlerTests
{
    ...
    public async Task Handle_MapsEntitiesToDtosPreservingOrderAndFields()
    {
        ...
        ChatMessage first = ChatMessage.Create(roomId, MessageAuthor.User("u1", "Ana"), "first", sentAt);
        ChatMessage second = ChatMessage.Create(roomId, MessageAuthor.Bot(), "second", sentAt.AddMinutes(1));
